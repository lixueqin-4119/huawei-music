...
class Player {
    ...
    bind() {
        ...
        this.$('.btn-play-pause').onclick = function () {
            //self.audio.src = self.songList[self.currentIndex].url 不能放这，歌曲会重置
    ...
    loadSong() {
        ...
        this.audio.onloadedmetadata = () => this.$('.time-end').innerText = this.formateTime(this.audio.duration)//获取总时长
        this.loadLyric()
    }
    ...
    setLyricToCenter(node) {
        ...
        translateY = translateY > 0 ? translateY : 0 //>0时才偏移
    ...
    setProgressBar() { //设置进度条
        ...
        let percent = (this.audio.currentTime * 100 / this.audio.duration) + '%' //得到当前的百分比
        console.log(percent)
        this.$('.bar .progress').style.width = percent
        this.$('.time-start').innerText = this.formateTime(this.audio.currentTime) //处理格式
